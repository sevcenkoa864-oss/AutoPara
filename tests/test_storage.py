...
from autopara.core.models import STATUS_MANUAL, STATUS_OPENED, Lesson
...
class TestImport:
    ...
    def test_reimport_preserves_manual_lessons_and_occurrences(self, imported, courses):
        course_i = imported.courses()[0]
        group = imported.groups(course_i.id)[0]
        ...
        assert imported.occurrence(existing.id, today).status == STATUS_OPENED
